...
public class jsyshServer {
	...
	private String findsql(String power_option){
		if(power_option!=null&&power_option.length()>0&&!power_option.equals("null")){		
			...
			if(a[0].equals("1")){
				...
				sql += " and c.comid in ("+is.substring(0, is.length()-1)+")";
	...
	public List<Map<String, Object>> findsh(String postData,String power_option){
		...
		String sql ="select c.* from (select t.*,b.* from (select * from tb_driver_register x where add_time = (select max(add_time) from tb_driver_register y where x.id_card=y.id_card) or x.open_id is not null ) t";
		sql += " left join (select veh.vehicle_no vehicle,veh.type,cow.owner_name,com.province,com.city,com.company_name,com.id comid,cow.id cowid,veh.id vehid"
				+ " from TB_CAR_OWNER cow,tb_company com,tb_vehicle veh "
				+ "where veh.OWNER_ID=cow.ID and cow.company_id=com.id  ";
		sql +=" ) b on b.vehicle=t.vehicle_no order by case when t.issh='2' then 1 when t.issh='0' then 2 when t.issh='1' then 3 when t.issh='3' then 4 end,add_time desc) c where 1=1" ;
	...
	public String findshb(int pageIndex,int pageSize,String postData,String power_option){
		...
		String choosed = String.valueOf(paramMap.get("choosed"));
		String issh = String.valueOf(paramMap.get("issh"));
		String sql ="select a.* from ( select rownum rn ,c.* from (select t.*,b.* from (select * from tb_driver_register x where add_time = (select max(add_time) from tb_driver_register y where x.id_card=y.id_card) or x.open_id is not null ) t";
		sql += " left join (select veh.vehicle_no vehicle,veh.type,cow.owner_name,com.province,com.city,com.company_name,com.id comid,cow.id cowid,veh.id vehid"
				+ " from TB_CAR_OWNER cow,tb_company com,tb_vehicle veh "
				+ "where veh.OWNER_ID=cow.ID and cow.company_id=com.id ";
		sql +=" ) b on b.vehicle=t.vehicle_no order by case when t.issh='2' then 1 when t.issh='0' then 2 when t.issh='1' then 3 when t.issh='3' then 4 end,add_time desc) c where 1=1" ;
